import firebase, { getDatabase, getFirebaseAnalytics } from './firebase';
...
(window as any).joinRoom = async function () {
    ...
    try {
        ...
        if (roomData.status === GameStatus.PLAYING) {
...
(window as any).startGame = async function () {
    ...
    try {
        ...
        currentRoom!.secretWord = word;
        currentRoom!.selectedPlayerId = selectedPlayerId;
        currentRoom!.startTime = Date.now();
        currentRoom!.duration = 180; 

        await database.ref(`rooms/${currentRoom?.code}`).update(currentRoom!);
...
function startGameScreen(roomData: any): void {
    ...
    database.ref(`rooms/${currentRoom!.code}/submissions`).on('value', (snapshot) => {
...
(window as any).submitWord = async function () {
    ...
    try {
        await database.ref(`rooms/${currentRoom!.code}/submissions/${currentPlayer.id}`).set({
...
(window as any).leaveRoom = async function () {
    ...
    try {
        ...
        // Clean up listeners
        database.ref(`rooms/${currentRoom!.code}`).off();
        database.ref(`rooms/${currentRoom!.code}/submissions`).off();
